    ...
    def __init__(self, batch_size=8, split='train',
        ...
        for i_item, item in enumerate(self.ori_raw_data):
            ...
            if tokenizer is None:
                instruction_tokens = _encode_instruction_without_baseline_tokenizer(
...
def _encode_instruction_without_baseline_tokenizer(text: str, max_len: int):
    tokens = np.zeros((max_len,), dtype=np.int64)
    words = str(text).lower().replace(",", " ").replace(".", " ").split()
    for idx, word in enumerate(words[:max_len]):
        tokens[idx] = abs(hash(word)) % 10000 + 1
    return tokens
